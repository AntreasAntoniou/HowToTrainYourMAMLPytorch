    ...
    for dataset_idx, dataset_path in enumerate(dataset_paths):
        ...
        if not os.path.exists(dataset_path):
            ...
            zip_directory = "{}.tar.bz2".format(
                os.path.join(os.environ["DATASET_DIR"], datasets[dataset_idx])
            )

            assert os.path.exists(os.path.abspath(zip_directory)), (
                "{} dataset zip file not found"
                "place dataset in datasets folder as explained in README".format(
                    os.path.abspath(zip_directory)
                )
            )
            print("Found zip file, unpacking")

            unzip_file(
                filepath_pack=os.path.join(
                    os.environ["DATASET_DIR"],
                    "{}.tar.bz2".format(datasets[dataset_idx]),
                ),
        ...
        for subdir, dir, files in os.walk(dataset_path):
            for file in files:
                if (
                    ...
                    or file.lower().endswith(".pkl")
                ):
                    total_files += 1
        print(
            "count stuff________________________________________", total_files
        )
        if (
            (
                total_files == 1623 * 20
                and datasets[dataset_idx] == "omniglot_dataset"
            )
            or (
                total_files == 100 * 600
                and "mini_imagenet" in datasets[dataset_idx]
            )
            or (
                total_files == 3
                and "mini_imagenet_pkl" in datasets[dataset_idx]
            )
        ):
...
def unzip_file(filepath_pack, filepath_to_store):
    command_to_run = "tar -I pbzip2 -xf {} -C {}".format(
        filepath_pack, filepath_to_store
    )
