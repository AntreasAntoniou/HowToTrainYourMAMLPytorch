...
def get_best_validation_model_statistics(
    ...
    """
    log_file_dict = load_statistics(
        filename=filename, experiment_name=experiment_name
    )
    d_val_loss = np.array(
        log_file_dict["total_d_val_loss_mean"], dtype=np.float32
    )
...
def create_json_experiment_log(
    ...
    experiment_summary_dict["experiment_status"] = [
