...
maml_experiment_script = "src/run.py"
prefix = "few_shot"
...
def fill_template(template_list, execution_script, experiment_config):
    ...
    execution_line = execution_line.replace(
...
for subdir, dir, files in os.walk(experiment_json_dir):
    for file in files:
        if file.endswith(".json"):
            ...
            local_script_name = "../{}/{}_{}.sh".format(
                "experiment_scripts", file.replace(".json", ""), prefix
            )
            local_script_name = os.path.abspath(local_script_name)
            write_text_to_file(
                text=local_script_text, filepath=local_script_name
            )
