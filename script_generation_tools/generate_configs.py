...
hyper_config = namedtuple(
    "hyperconfig",
    "num_samples_per_class_range "
    "batch_size_range "
        ...
        for named_config in named_configs:
            ...
            configs_list.append(
                config(
                    ...
                    if experiment_dataset_name == "mini-imagenet"
                    else 1,
                    batch_size=named_config["batch_size"],
                    train_update_steps=5,
                    val_update_steps=5,
                    init_inner_loop_learning_rate=named_config[
                        "init_inner_loop_learning_rate"
                    ],
                    load_into_memory=True,
                    learnable_bn_gamma=True,
                    learnable_bn_beta=True,
                    num_filters=named_config["num_filters"],
                    conv_padding=True,
                )
            )
    ...
    for template_file in files:
        for seed_idx in seed_list:
            ...
            for config_item in configs_list:
                ...
                config_item["experiment_name"] = "{}_{}".format(
                    config_item["experiment_name"], config_item["train_seed"]
                )
